        ...
        extends AbstractSet<E> 
{
    ...
    protected Entry<E> deleteEntry (Entry<E> p) 
    {
        ...
        if (p.left.IsExternal() == false && p.right.IsExternal() == false) 
        {
        ...
        if (p.left.IsExternal() == false)
            replacement = p.left;
        else
            replacement = p.right;

        // If p has at least one child, link replacement to p.parent.
        if (replacement.IsExternal() == false) 
        {
    ...
    protected Entry<E> successor (Entry<E> e) 
    {
        if (e.IsExternal() == true)
            return null;
        else if (e.right.IsExternal() == false) 
        {
            // successor is leftmost Entry in right subtree of e
            Entry<E> p = e.right;
            while (p.left.IsExternal() == false)
                p = p.left;
            return p;
    ...
    protected class TreeIterator implements Iterator<E>
    {
        ...
        {             
            next = root;
            if (next != null && next.IsExternal() == false)
                while (next.left.IsExternal() == false)
                    next = next.left;
        ...
        public void remove() 
        {
            ...
            if (lastReturned.left.IsExternal() == false && lastReturned.right.IsExternal() == false)
                next = lastReturned;
    ...
    //I used the pages in the book suggested in the Coursework to help me with this 
    //section
    public String BreadthFirst(){
    	Queue<Entry<E>> queue = new LinkedList<Entry<E>>();
    	String BreadthFirst = new String();
    	...
    	if (root == null){
    		...
    		while (queue.isEmpty() == false){
    			...
    			Entry<E> current = queue.remove();
    			BreadthFirst = BreadthFirst + current.element + "\n";
    			if (current.left.IsExternal() == false){
    				queue.add(current.left);
    			}
    			if (current.right.IsExternal() == false){
    ...
    public int treeHeight(){
    	...
    	while(0==0){
    		...
    		while (counter > 0){
    			Entry<E> CompareEntry = Tracker.peek();
    			
    			Tracker.remove();
    			
    			if (CompareEntry.left.IsExternal() == false){
    				Tracker.add(CompareEntry.left);
    			}
    			
    			if (CompareEntry.right.IsExternal() == false){
    				Tracker.add(CompareEntry.right);
    			}
    ...
    protected static class Entry<E> 
    {
        ...
        //Constructor For Internal Nodes
         ...
         //Converts an Existing External Node to an Internal Node
		...
		//Converts an Existing Internal Node to an External Node and Returns the Nodes Element
         ...
         //Checks if a Node is Internal or External
